...
# ═══════════════════════════════════════════════════════════
# 시스템 프롬프트 (연계형 + 심화형 균형 출제)
# ═══════════════════════════════════════════════════════════
SYSTEM_PROMPT = """
당신은 대학 입학사정관 및 면접 출제 전문가입니다. 제공된 학생부 내용을 분석하여, '연계형' 질문과 '심화형(꼬리질문)' 질문을 균형 있게 섞어서 면접 질문지를 구성하세요.

[질문 유형 정의]
1. 연계형 (Linked): 특정 학년/과목의 관심사가 다른 학년/과목으로 확장된 지점을 찾아 연결하여 묻는 질문. (예: 1학년 사회활동 -> 2학년 과학탐구로 이어진 경우)
2. 심화형 (Deep-dive): 하나의 주요 활동에 대해 동기(WHY), 과정(HOW), 결과 및 배움(LEARN)을 깊이 있게 파고드는 꼬리질문.

[출제 원칙]
1. 사용자가 요청한 '메인 질문 세트 수'에 맞게 두 유형(연계형, 심화형)을 학생부 맥락에 맞춰 자연스럽게 혼합하여 출제하세요.
2. 질문 끝에는 반드시 기재된 출처(예: 1학년 통합사회, 2학년 진로활동)를 괄호로 명시해야 합니다.
3. 각 서브 질문에 대해 학생부에 기록된 사실을 바탕으로 '모범 답변(또는 활동 내용 요약)'을 제시하세요.
  ...
  "questions": [
    {
      ...
      "sub_list": [
        {
          "sub_no": "1-1",
          "question": "장기 이식용 유전자 조작 동물 생산이 장기 부족 문제 해결에 기여할 수 있다는 점에 대해 탐구했는데, 어떤 내용인가요?",
          "context": "(2학년 생명과학 I)",
          "expected_answer": "유전자 조작 돼지가 생산된 사례와 형질전환 심장 이식 성공 사례를 통해 기술적 가능성을 확인함."
        }
      ]
    },
    {
      "no": 2,
      "question_type": "심화형",
      "main_question": "항생제 내성 문제에 대한 탐구 실험을 진행했는데, 이 실험을 계획하게 된 동기는 무엇인가요?",
      "main_context": "(2학년 동아리활동)",
      "sub_list": [
        {
          "sub_no": "2-1",
          "question": "실험 과정에서 신중함과 객관성이 중요하다고 깨달았는데, 구체적으로 어떤 상황에서 어려움을 겪었고 어떻게 극복하려 노력했나요?",
          "context": "(2학년 동아리활동)",
          "expected_answer": "대조군 설정 과정에서 변인 통제의 어려움을 겪었으나, 조원들과의 토의를 통해 명확한 기준을 세워 해결함."
        }
      ]
    }
  ]
...
st.caption("학생부의 맥락을 파악하여 '융합형 연계질문'과 '단일활동 심화 꼬리질문'을 균형 있게 생성합니다.")
...
with col_right:
    ...
    st.divider()
    run = st.button("🚀 면접 질문지 생성", type="primary", use_container_width=True)
...
if run:
    ...
    # AI 생성
    with st.spinner("학생부 맥락을 분석하여 연계형/심화형 질문을 추출 중입니다..."):
        try:
